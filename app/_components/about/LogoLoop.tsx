        ...
        const renderLogoItem = useCallback(
            (item: LogoItem, key: React.Key) => {
                ...
                if (isNodeItem) {
                    ...
                    // Fix hydration mismatch: server renders 'dark' (or default), client waits for mount to switch to 'light' if needed
                    const themeParam = (mounted && resolvedTheme === 'light') ? 'light' : 'dark';
                    const originalSrc = (item as any).src;
                    const finalSrc = typeof originalSrc === 'string' && originalSrc.includes('skillicons.dev')
                        ? `${originalSrc}${originalSrc.includes('?') ? '&' : '?'}theme=${themeParam}`
                        : originalSrc;
                ...
                const contentWithTitle = (
                    <div className="flex items-center gap-3 py-4">
                        {content}
                        {itemTitle && (
                            <span className="text-lg font-semibold text-textPrimary whitespace-nowrap opacity-80 group-hover:opacity-100 transition-opacity">
                                {itemTitle}
                            </span>
                        )}
                    </div>
            ...
            [isVertical, scaleOnHover, renderItem, resolvedTheme]
        );
